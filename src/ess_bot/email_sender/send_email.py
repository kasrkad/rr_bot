...
def send_email_with_screenshot(duty=None, sender_email=EMAIL_SENDER_LOGIN, sender_password=EMAIL_SENDER_PASS):
  """Отправляем скриншот с работами на почту,
    [duty]:Текущий дежурный
    [sender_email]:Адрес с которого отправляется сообщение
    [sender_password]:Пароль для учетной записи с которой отправляется сообщение"""
