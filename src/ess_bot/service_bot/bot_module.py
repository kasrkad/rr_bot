...
from .service_config import ESS_CHAT_ID, PPAK_STANDBY_CONNECTION_STRING,\
    DEFAULT_ORACLE_PASS, DEFAULT_ORACLE_USER, PPAK_SOAP_URL,\
    DEV_SOAP_URL, DEV_DOC_CONNECTION_STRING
...
class Ess_service_bot:
    ...
    def bot_commads(self):
        ...
        def start_command(message):
            service_bot_logger.info('Инициировано главное меню')
            try:
                self.bot.send_message(
                    message.from_user.id, 'Меню',
                    reply_markup=keyboards.main_menu_keyboard)
            except Exception:
                service_bot_logger.error(
                    'Ошибка при отображении главного меню')
...
""")
        ...
        def reqister_duty_engeneer(message, tg_id=None):
            try:
                ...
                service_bot_logger.info(
                    f'Запрос на установку дежурного с id {message.from_user.id}')
                ...
                asterisk_lib.set_duty_phone(
                    sql_lib.return_phone_num_db(tg_id_for_set))
...
новый дежурный - [{new_duty["fio"]}](tg://user?id={new_duty["tg_id"]})')
                ...
                service_bot_logger.error(
                    f'Ошибка при установке дежурного {exc.args}')
                self.bot.send_message(
                    message.from_user.id, 'Не удалось установить нового дежурного')
        ...
        def set_duty_dialogue(message):
            ...
            service_bot_logger.info(
                f'От {message.from_user.id} запрошено ручное переключение дежурного.')
            try:
                show_all_admins(message)
                message_for_set_duty = self.bot.send_message(
                    message.from_user.id, 'Введите телеграм ID для установки дежурного в ручную')
                self.bot.register_next_step_handler(
                    message_for_set_duty, register_duty_engeneer_manual)
            except Exception:
                self.bot.send_message(
                    message.from_user.id, 'Произошла ошибка в диалоге.')
                service_bot_logger.error(
                    f'Произошла ошибка в диалоге установки дежурного в ручную от {message.from_user.id}.', exc_info=True)
...
новый дежурный - [{new_duty["fio"]}](tg://user?id={new_duty["tg_id"]})')
        ...
        def set_owner_dialogue(message):
            ...
            service_bot_logger.info(
                f'От {message.from_user.id} запрошено ручное переключение руководителя группы.')
            try:
                show_all_admins(message)
                message_for_set_duty = self.bot.send_message(
                    message.from_user.id, 'Введите телеграм ID для установки ответственного за HPSM в ручную')
                self.bot.register_next_step_handler(
                    message_for_set_duty, set_owner_manual)
            except Exception:
                self.bot.send_message(
                    message.from_user.id, 'Произошла ошибка в диалоге.')
                service_bot_logger.error(
                    f'Произошла ошибка в диалоге установки руководителя группы в ручную от {message.from_user.id}.', exc_info=True)
        ...
        def set_owner_manual(message):
            ...
            try:
                ...
                if current_owner:
                    self.bot.send_message(
                        message.from_user.id, f'Предыдущий ответственный за HPSM [{current_owner["fio"]}](tg://user?id={current_owner["tg_id"]})')
                else:
                    self.bot.send_message(
                        message.from_user.id, 'Предыдущий ответственный за HPSM не обнаружен.')
                sql_lib.set_owner_or_duty_db(
                    message.text.strip(), role='owner')
                new_owner = sql_lib.get_owner_or_duty_db(role='owner')
                self.bot.send_message(
                    message.from_user.id, f'Новый ответственный за HPSM - [{new_owner["fio"]}](tg://user?id={new_owner["tg_id"]})')

            except Exception:
                self.bot.send_message(
                    message.from_user.id, 'Произошла в установке нового ответственного.')
                service_bot_logger.error(
                    f'Произошла ошибка при установке HPSM_owner = {message.text}', exc_info=True)
        ...
        def producer_recreate(message):
            ...
            try:
                ...
                service_bot_logger.error(
                    f'Произошла ошибка при пересоздании продюсера, запрос от {message.from_user.id}.', exc_info=True)

        @self.bot.message_handler(commands=['contacts'])
        def get_contacts(message):
            service_bot_logger.info(
                f'Пользователем {message.from_user.id} запрошены контакты.')
            try:
                current_duty = sql_lib.get_owner_or_duty_db(role='duty')
                current_owner = sql_lib.get_owner_or_duty_db(role='owner')
...
Координатор СИМИ,ОНКО,СЭМД - [{current_owner["fio"]}](tg://user?id={current_owner["tg_id"]})''')
                ...
                self.bot.send_message(
                    message.from_user.id, f'Произошла ошибка при запросе контактов.')
                service_bot_logger.error(
                    f'Произошла ошибка при запросе контактов от {message.from_user.id}', exc_info=True)

        @self.permissions_decorator
        @self.bot.message_handler(commands=['notify_get'])
        def notify_get(message):
            service_bot_logger.info(
                f'Пользователем {message.from_user.id} запрошены уведомления.')
            try:
                ...
                if message_with_notifys:
                    self.bot.send_message(
                        message.from_user.id, "Список доступных уведомлений.")
                    self.bot.send_message(
                        message.from_user.id, message_with_notifys, parse_mode="HTML")
                    return
                self.bot.send_message(
                    message.from_user.id, 'Уведомлений не обнаружено')
            except Exception:
                self.bot(message.from_user.id,
                         'Произошла ошибка при запросе уведомлений.')
                service_bot_logger.error(
                    f'Произошла ошибка при запросе уведомлений от {message.from_user.id}', exc_info=True)

        @self.permissions_decorator
        @self.bot.message_handler(commands=['change_notify_status'])
        def change_notify_status_dialogue(message):
            service_bot_logger.info(
                f'Пользовтелем {message.from_user.id} запрошено изменение состояния уведомления.')
            notify_get(message)
            message_for_set_status = self.bot.send_message(
                message.from_user.id, 'Введите id уведомлений, и желаемый статус(on/off), через : .Пример - 1:off')
            self.bot.register_next_step_handler(
                message_for_set_status, set_notify_status)
        ...
        def set_notify_status(message):
            ...
            try:
                ...
                self.bot.send_message(
                    message.from_user.id, 'Статус уведомления успешно изменен.')
            except Exception:
                service_bot_logger.info(
                    f'Произошла ошибка при изменении состояния уведомления {message.text}')
                self.bot.send_message(
                    message.from_user.id, 'Произошла ошибка при изменении состояния уведомления.')

        @self.bot.message_handler(regexp=self.document_regexp)
        def document_handle(message):
            """Ищем guid документа по регулярке"""
            global finded_docs
            finded_docs = re.findall(
                self.document_regexp, message.text.strip())
            if sql_lib.check_admin_permissions(message.from_user.id):
            ...
            def callback_inline(call):
                try:
                    ...
                    if call.data == 'download_doc_test':
                        ...
                        documents_for_send = write_json_or_xml_document(
                            doc_data=doc_data_test)
                        send_file(tg_id=call.from_user.id,
                                  files=documents_for_send)
                    if call.data == 'download_json_test':
                        ...
                        documents_for_send = write_json_or_xml_document(
                            doc_data=doc_data_json_test, file_format='json')
                        send_file(tg_id=call.from_user.id,
                                  files=documents_for_send)
                    ...
                    if call.data == 'status_test':
                        ...
                        for document, status in documents_with_statuses.items():
                            self.bot.send_message(
                                call.from_user.id, f'{document}: {status}')
                    ...
                    self.bot.send_message(
                        call.from_user.id, 'Документы на стенде не обнаружены.')
                except Exception:
                    service_bot_logger.error(
                        'Произошла ошибка при работе с документами', exc_info=True)
                    self.bot.send_message(
                        call.from_user.id, 'Вовремя операции с документом произошла ошибка.')
        ...
        def send(message):
            try:
                self.bot.send_message(
                    message.from_user.id, 'Отправляем скриншот')
                send_email_with_screenshot(
                    duty=sql_lib.get_owner_or_duty_db(role='duty')['fio'])
            except FileNotFoundError:
                self.bot.send_message(
                    message.from_user.id, 'Файл для отправки не обнаружен, в админпанели нажмите отправить скриншот.')
            except Exception:
                service_bot_logger.error(
                    'Произошла ошибка при отправке скриншота.')
                self.bot.send_message(
                    message.from_user.id, 'Отправка скриншота не удалась.')

        @self.bot.message_handler(commands=['status'])
        @self.permissions_decorator
        def status(message):
            hpsm_status = sql_lib.get_hpsm_status_db()
            time_from_last_check = datetime.fromtimestamp(
                hpsm_status['timestamp'])
...
Дежурный и координатор доступны по команде /contacts')
        ...
        @self.bot.message_handler(commands=['hpsmPanel'])
        @self.permissions_decorator
        def show_hpsm_panel(message):
            service_bot_logger.info(
                f'Открыта hspm панель для пользователя {message.from_user.id}')
            try:
                self.bot.send_message(
                    message.from_user.id, 'Открыта HPSM панель.',
                    reply_markup=keyboards.hpsm_checker_keyboard)
            except Exception:
                service_bot_logger.error(
                    f'Произошла ошибка при открытии HPSM панели для {message.from_user.id}')
                self.bot.send_message(
                    message.from_user.id, 'Произошла ошибка при открытии HPSM панели.')

        @self.bot.message_handler(commands=['adminPanel'])
        @self.permissions_decorator
        def show_admin_panel(message):
            service_bot_logger.info(
                f'Открыта админ панель для пользователя {message.from_user.id}')
            try:
                self.bot.send_message(
                    message.from_user.id, 'Открыта админ панель.', reply_markup=keyboards.admin_menu_keyboard)
            except Exception:
                service_bot_logger.error(
                    f'Произошла ошибка при открытии административной панели для {message.from_user.id}')
                self.bot.send_message(
                    message.from_user.id, 'Произошла ошибка при открытии административной панели.')
        ...
        def callback_inline(call):
            try:
                if call.data == 'show_hpsm_panel':
                    show_hpsm_panel(call)
                ...
                if call.data == 'screenshot':
                    self.bot.send_message(
                        call.from_user.id, 'Запрошен скриншот, это может занять до 30 секунд.')
                    service_bot_logger.info(
                ...
                service_bot_logger.error(
                    'Во время операции произошла ошибка', exc_info=True)
                self.bot.send_message(
                    call.from_user.id, 'Во время операции произошла ошибка.', reply_markup=keyboards.admin_menu_keyboard)
    ...
    def run(self):
        while True:
            try:
                ...
                service_bot_logger.error(
                    'Ошибка при запуске бота', exc_info=True)
                sleep(30)
