...
class Hpsm_checker(Thread):
        ...
        while True:
            try:
                ...
                if data[1] == 'notification_off':
                    ...
                    self.send_notification(text='Уведомления о заявках отключены.',channel=True)
                    self.send_notifi_flag = False
                if data[1] == 'notification_on':
                    hpsm_logger.info('Уведомления HPSM включены от {data[0]}')
                    self.send_notifi_flag = True
                    self.send_notification(text='Уведомления о заявках включены', channel=True)
                if data[1] == 'screenshot':
                    hpsm_logger.info('В очереди пришла команда на снятие скриншота')
    ...
    def login_hpsm(self, webdriver, user, password):
        ...
        try:
            ...
            webdriver.quit()
            hpsm_logger.error(f'Произошла ошибка при логине с учетными данными {user}',exc_info=True)
            print(exc.args)
    ...
    def get_html_page(self, webdriver)-> str:
        ...
        webdriver_login = self.login_hpsm(webdriver,user=HPSM_USER, password=HPSM_PASS)
