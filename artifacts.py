	...
	def start_cli(self,message):
		...
		if answer.strip() == 'Y':
			...
			try:
				...
				self.bot.send_message(message.from_user.id,"Загрузка успешно завешена")
...
class Artifact:
	...
	def __init__(self, stand=None, task=None, commit=None):
		self.stand=stand
		self.task=task
		self.commit=commit
	...
	def remove_folders(self):
		...
		print(f"in remove function {dirs_for_remove}")
	...
	def write_activity_log(self, action):
		with open("./cct/cct_logs/activity.json") as activity_file:
			json.dump({"time": datetime.datetime.today().strftime("%d-%m-%Y"), "stand":self.stand, "action":action ,"task":self.task}, activity_file,ensure_ascii=False)
			activity_file.write("\n")
	...
	def dump_artifact(self):
		...
		for stand_name, stand_settings in self.settings_loader().items():
			...
			break
		process = Popen(['bash','./test.sh'," ".join(arg for arg in stand_set)], stdout=PIPE, stderr=PIPE)
		process.wait()
